...
export const approvalReportHasImage = mysqlTable("approval_report_has_image", {
  approvalReportId: int("approval_report_id", { unsigned: true }).notNull()
    .references(() => approvalReport.id, { onDelete: "cascade" }),
...
export const documentCategory = mysqlTable("document_category", {
  ...
  documentType: int("document_type").notNull().references(() =>
    documentTypes.id
  ),
...
export const forecast = mysqlTable("forecast", {
  id: int().autoincrement().notNull(),
  itemNo: varchar("item_no", { length: 20 }).notNull().references(() =>
    sItem.no
  ),
  colorCode: varchar("color_code", { length: 10 }).notNull(),
  amount: int().notNull(),
  timestamp: timestamp({ mode: "string" }).default(sql`(now())`).notNull(),
  countryCode: varchar("country_code", { length: 10 }).notNull().references(
    () => sCountry.code,
  ),
  createdBy: int("created_by", { unsigned: true }).notNull().references(() =>
    user.id
  ),
...
export const itemComment = mysqlTable("item_comment", {
  ...
  commentBy: int("comment_by", { unsigned: true }).notNull().references(() =>
    user.id
  ),
  purchasingRole: int("purchasing_role", { unsigned: true }).references(() =>
    role.id
  ),
  timestamp: timestamp({ mode: "string" }).default(sql`(now())`).notNull(),
}, (table) => {
...
export const loadingList = mysqlTable("loading_list", {
  ...
  transportType: int("transport_type", { unsigned: true }).notNull().references(
    () => transportType.id,
  ),
  ...
  creatorUserId: int("creator_user_id", { unsigned: true }).references(() =>
    user.id
  ),
  ...
  dateCreated: timestamp("date_created", { mode: "string" }).default(
    sql`(now())`,
  ).notNull(),
...
export const loadingListReservedCalendarTime = mysqlTable(
  ...
  {
    id: int({ unsigned: true }).autoincrement().notNull(),
    loadingListId: int("loading_list_id").references(() => loadingList.id, {
      onDelete: "restrict",
      onUpdate: "restrict",
    }),
    countryCode: varchar("country_code", { length: 20 }).references(
      () => sCountry.code,
...
export const menuHasPermission = mysqlTable("menu_has_permission", {
  ...
  permissionId: int("permission_id", { unsigned: true }).notNull().references(
    () => permission.id,
...
export const menuHasRole = mysqlTable("menu_has_role", {
  menuId: smallint("menu_id", { unsigned: true }).notNull().references(
    () => menu.id,
    { onDelete: "cascade", onUpdate: "restrict" },
  ),
...
export const technicalReportHasImage = mysqlTable(
  "technical_report_has_image",
  {
    technicalReportId: int("technical_report_id", { unsigned: true })
      .references(() => technicalReport.id, { onDelete: "cascade" }),
    base64Image: longtext("base64_image"),
  },
  (table) => {
    return {
...
export const userHasRole = mysqlTable("user_has_role", {
  userId: int("user_id", { unsigned: true }).notNull().references(
    () => user.id,
    { onDelete: "cascade", onUpdate: "restrict" },
  ),
  roleId: int("role_id", { unsigned: true }).notNull(),
