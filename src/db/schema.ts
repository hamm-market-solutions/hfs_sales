import { mysqlTable, mysqlSchema, AnyMySqlColumn, index, primaryKey, varchar, timestamp, foreignKey, int, unique, smallint, bigint, date } from "drizzle-orm/mysql-core"
...
export const sItemColor = mysqlTable("s_item_color", {
	itemNo: varchar("item_no", { length: 20 }).notNull().references(() => sItem.no, { onDelete: "cascade", onUpdate: "cascade" } ),
	colorCode: varchar("color_code", { length: 10 }).notNull(),
...
export const userHasCountry = mysqlTable("user_has_country", {
	userId: int("user_id", { unsigned: true }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	countryCode: varchar("country_code", { length: 10 }).notNull().references(() => sCountry.code, { onDelete: "cascade", onUpdate: "cascade" } ),
},
(table) => {
	return {
		countryCode: index("country_code").on(table.countryCode),
