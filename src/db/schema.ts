import { mysqlTable, mysqlSchema, AnyMySqlColumn, primaryKey, int, varchar, index, foreignKey, text, tinyint, timestamp, datetime, longtext, unique, tinytext, double, mediumtext, smallint, date, time, bigint } from "drizzle-orm/mysql-core"
...
(table) => {
	return {
		adminSettingsId: primaryKey({ columns: [table.id], name: "admin_settings_id"}),
	}
});

export const approvalReport = mysqlTable("approval_report", {
	id: int({ unsigned: true }).autoincrement().notNull(),
...
(table) => {
	return {
		...
		approvalReportId: primaryKey({ columns: [table.id], name: "approval_report_id"}),
	}
});

export const approvalReportHasImage = mysqlTable("approval_report_has_image", {
	approvalReportId: int("approval_report_id", { unsigned: true }).notNull().references(() => approvalReport.id, { onDelete: "cascade" } ),
...
export const carton = mysqlTable("carton", {
	id: int({ unsigned: true }).autoincrement().notNull(),
...
(table) => {
	return {
		...
		uniqueKey: unique("unique_key").on(table.length, table.width, table.height, table.weight, table.creatorUserId, table.vendorNo),
	}
});

export const direction = mysqlTable("direction", {
	id: int().notNull(),
...
export const documentCategory = mysqlTable("document_category", {
	id: int({ unsigned: true }).autoincrement().notNull(),
...
(table) => {
	return {
		documentCategoryId: primaryKey({ columns: [table.id], name: "document_category_id"}),
	}
});
...
(table) => {
	return {
		documentTypesId: primaryKey({ columns: [table.id], name: "document_types_id"}),
...
export const itemComment = mysqlTable("item_comment", {
	id: int({ unsigned: true }).autoincrement().notNull(),
	itemNo: varchar("item_no", { length: 20 }).notNull(),
...
(table) => {
	return {
		...
		itemCommentId: primaryKey({ columns: [table.id], name: "item_comment_id"}),
	}
});

export const loadingList = mysqlTable("loading_list", {
	id: int().autoincrement().notNull(),
	seasonCode: smallint("season_code").references(() => sSeason.code),
	status: smallint().notNull(),
	direction: int().default(0).notNull().references(() => direction.id),
	transportType: int("transport_type", { unsigned: true }).notNull().references(() => transportType.id),
	containerCode: varchar("container_code", { length: 20 }),
	vendorNo: varchar("vendor_no", { length: 20 }).references(() => sVendor.no),
	vendorReference: varchar("vendor_reference", { length: 50 }),
	vendorComment: mediumtext("vendor_comment"),
	creatorUserId: int("creator_user_id", { unsigned: true }).references(() => user.id),
...
(table) => {
	return {
		loadingListId: primaryKey({ columns: [table.id], name: "loading_list_id"}),
	}
});

export const loadingListCalendarTime = mysqlTable("loading_list_calendar_time", {
	id: int({ unsigned: true }).notNull(),
	time: time(),
});

export const loadingListComments = mysqlTable("loading_list_comments", {
	id: int({ unsigned: true }).autoincrement().notNull(),
...
(table) => {
	return {
		loadingListCommentsId: primaryKey({ columns: [table.id], name: "loading_list_comments_id"}),
	}
});

export const loadingListDocument = mysqlTable("loading_list_document", {
	id: int({ unsigned: true }).autoincrement().notNull(),
...
(table) => {
	return {
		loadingListDocumentId: primaryKey({ columns: [table.id], name: "loading_list_document_id"}),
	}
});

export const loadingListLine = mysqlTable("loading_list_line", {
	id: int({ unsigned: true }).autoincrement().notNull(),
	loadingListId: int("loading_list_id").notNull(),
	creatorUserId: int("creator_user_id"),
...
(table) => {
	return {
		loadingListLineId: primaryKey({ columns: [table.id], name: "loading_list_line_id"}),
	}
});

export const loadingListReservedCalendarTime = mysqlTable("loading_list_reserved_calendar_time", {
	id: int({ unsigned: true }).autoincrement().notNull(),
	loadingListId: int("loading_list_id").references(() => loadingList.id, { onDelete: "restrict", onUpdate: "restrict" } ),
	countryCode: varchar("country_code", { length: 20 }).references(() => sCountry.code, { onDelete: "restrict", onUpdate: "restrict" } ),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
...
(table) => {
	return {
		...
		countryCode: index("country_code").on(table.countryCode),
		loadingListReservedCalendarTimeId: primaryKey({ columns: [table.id], name: "loading_list_reserved_calendar_time_id"}),
	}
});

export const loadingListSsccFile = mysqlTable("loading_list_sscc_file", {
	id: int({ unsigned: true }).autoincrement().notNull(),
	loadingListId: int("loading_list_id").notNull(),
	creatorUserId: int("creator_user_id"),
...
(table) => {
	return {
		loadingListSsccFileId: primaryKey({ columns: [table.id], name: "loading_list_sscc_file_id"}),
	}
});
...
export const menuHasPermission = mysqlTable("menu_has_permission", {
	menuId: int("menu_id", { unsigned: true }).notNull(),
	permissionId: int("permission_id", { unsigned: true }).notNull().references(() => permission.id, { onDelete: "cascade", onUpdate: "restrict" } ),
},
(table) => {
	return {
		menuHasPermissionMenuIdPermissionId: primaryKey({ columns: [table.menuId, table.permissionId], name: "menu_has_permission_menu_id_permission_id"}),
		menuHasPermissionMenuIdPermissionIdUindex: unique("menu_has_permission_menu_id_permission_id_uindex").on(table.menuId, table.permissionId),
	}
});

export const menuHasRole = mysqlTable("menu_has_role", {
	menuId: smallint("menu_id", { unsigned: true }).notNull().references(() => menu.id, { onDelete: "cascade", onUpdate: "restrict" } ),
	roleId: int("role_id", { unsigned: true }).notNull(),
},
(table) => {
	return {
		roleFk: index("role___fk").on(table.menuId),
		menuHasRoleMenuIdRoleId: primaryKey({ columns: [table.menuId, table.roleId], name: "menu_has_role_menu_id_role_id"}),
		menuHasRoleMenuIdRoleIdUindex: unique("menu_has_role_menu_id_role_id_uindex").on(table.menuId, table.roleId),
	}
});
...
export const qualities = mysqlTable("qualities", {
	id: int().autoincrement().notNull(),
	name: varchar({ length: 50 }).notNull(),
},
(table) => {
...
export const returns = mysqlTable("returns", {
	...
	qualityId: int("quality_id").notNull().references(() => qualities.id),
...
export const roleHasPermission = mysqlTable("role_has_permission", {
	roleId: int("role_id", { unsigned: true }).notNull(),
	permissionId: int("permission_id", { unsigned: true }).notNull(),
},
(table) => {
	return {
		roleHasPermissionRoleIdPermissionIdUindex: unique("role_has_permission_role_id_permission_id_uindex").on(table.roleId, table.permissionId),
	}
});

export const sAssortment = mysqlTable("s_assortment", {
...
export const sColor = mysqlTable("s_color", {
	code: varchar({ length: 10 }).notNull(),
...
(table) => {
	return {
		sCountryCode: primaryKey({ columns: [table.code], name: "s_country_code"}),
	}
});

export const sCustomer = mysqlTable("s_customer", {
	no: int().notNull(),
...
export const sDeliveryAddress = mysqlTable("s_delivery_address", {
	orderNo: varchar("order_no", { length: 20 }).notNull(),
...
(table) => {
	return {
		sDeliveryAddressOrderNo: primaryKey({ columns: [table.orderNo], name: "s_delivery_address_order_no"}),
	}
});
...
export const sItemColor = mysqlTable("s_item_color", {
	itemNo: varchar("item_no", { length: 20 }).notNull(),
	colorCode: varchar("color_code", { length: 10 }).notNull(),
	customColor: varchar("custom_color", { length: 30 }),
	purchasePrice: int("purchase_price"),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
	requestedExFactoryDate: date("requested_ex_factory_date", { mode: 'string' }),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
	firstCustomerShipmentDate: date("first_customer_shipment_date", { mode: 'string' }),
...
export const sPurchaseHead = mysqlTable("s_purchase_head", {
	orderNo: varchar("order_no", { length: 20 }).notNull(),
	vendorNo: varchar("vendor_no", { length: 20 }).notNull(),
...
(table) => {
	return {
		sPurchaseHeadOrderNo: primaryKey({ columns: [table.orderNo], name: "s_purchase_head_order_no"}),
	}
});

export const sPurchaseLine = mysqlTable("s_purchase_line", {
	id: int().autoincrement().notNull(),
	orderNo: varchar("order_no", { length: 20 }).notNull(),
	orderLineNo: int("order_line_no").notNull(),
	itemNo: varchar("item_no", { length: 20 }),
...
(table) => {
	return {
		...
		sPurchaseLineId: primaryKey({ columns: [table.id], name: "s_purchase_line_id"}),
...
export const sSeasonBrandPhase = mysqlTable("s_season_brand_phase", {
	seasonCode: smallint("season_code").notNull(),
	brandNo: varchar("brand_no", { length: 50 }),
	phase: smallint(),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
...
(table) => {
	return {
		uniqueKey: unique("unique_key").on(table.seasonCode, table.brandNo, table.phase),
	}
});
...
(table) => {
	return {
		...
		sStyleDescriptionUindex: unique("s_style_description_uindex").on(table.description),
	}
});

export const sVariant = mysqlTable("s_variant", {
	id: int({ unsigned: true }).autoincrement().notNull(),
	itemNo: varchar("item_no", { length: 20 }),
	colorCode: varchar("color_code", { length: 10 }),
...
(table) => {
	return {
		...
		uniqueKey: unique("unique_key").on(table.itemNo, table.colorCode, table.sizeCode),
	}
});

export const sVendor = mysqlTable("s_vendor", {
	no: varchar({ length: 20 }).notNull(),
	name: varchar({ length: 50 }),
...
export const shoeBox = mysqlTable("shoe_box", {
...
(table) => {
	return {
		...
		uniqueKey: unique("unique_key").on(table.length, table.width, table.height, table.weight, table.vendorNo),
	}
});

export const sscc = mysqlTable("sscc", {
...
export const ssccLine = mysqlTable("sscc_line", {
	id: int().autoincrement().notNull(),
	sscc: varchar({ length: 18 }).default('0').notNull(),
	orderNo: varchar("order_no", { length: 20 }).notNull(),
	orderLineNo: int("order_line_no").notNull(),
...
export const styleHasBox = mysqlTable("style_has_box", {
...
(table) => {
	return {
		styleHasBoxStyleCodeCategory: primaryKey({ columns: [table.styleCode, table.category], name: "style_has_box_style_code_category"}),
		styleHasBoxStyleCodeCategoryUindex: unique("style_has_box_style_code_category_uindex").on(table.styleCode, table.category),
	}
});

export const technicalReport = mysqlTable("technical_report", {
	id: int({ unsigned: true }).autoincrement().notNull(),
	itemNo: varchar("item_no", { length: 20 }).notNull(),
...
(table) => {
	return {
		creatorUserId: index("creator_user_id").on(table.creatorUserId),
		technicalReportId: primaryKey({ columns: [table.id], name: "technical_report_id"}),
	}
});

export const technicalReportHasImage = mysqlTable("technical_report_has_image", {
	technicalReportId: int("technical_report_id", { unsigned: true }).references(() => technicalReport.id, { onDelete: "cascade" } ),
	base64Image: longtext("base64_image"),
},
(table) => {
	return {
		technicalReportId: index("technical_report_id").on(table.technicalReportId),
	}
});

export const transportType = mysqlTable("transport_type", {
	id: int({ unsigned: true }).autoincrement().notNull(),
	name: varchar({ length: 20 }).notNull(),
},
(table) => {
	return {
		transportTypeId: primaryKey({ columns: [table.id], name: "transport_type_id"}),
	}
});
...
export const userHasPermission = mysqlTable("user_has_permission", {
	userId: int("user_id", { unsigned: true }).notNull(),
	permissionId: int("permission_id", { unsigned: true }).notNull().references(() => permission.id, { onDelete: "cascade", onUpdate: "restrict" } ),
},
(table) => {
	return {
		userHasPermissionUserIdPermissionId: primaryKey({ columns: [table.userId, table.permissionId], name: "user_has_permission_user_id_permission_id"}),
		userHasPermissionUserIdPermissionIdUindex: unique("user_has_permission_user_id_permission_id_uindex").on(table.userId, table.permissionId),
	}
});

export const userHasRole = mysqlTable("user_has_role", {
	userId: int("user_id", { unsigned: true }).notNull().references(() => user.id, { onDelete: "cascade", onUpdate: "restrict" } ),
	roleId: int("role_id", { unsigned: true }).notNull(),
},
(table) => {
	return {
		roleFk: index("role___fk").on(table.roleId),
		userHasRoleUserIdRoleId: primaryKey({ columns: [table.userId, table.roleId], name: "user_has_role_user_id_role_id"}),
		userHasRoleUserIdRoleIdUindex: unique("user_has_role_user_id_role_id_uindex").on(table.userId, table.roleId),
	}
});

export const userHasVendor = mysqlTable("user_has_vendor", {
	userId: int("user_id", { unsigned: true }).notNull().references(() => user.id, { onDelete: "cascade" } ),
	vendorNo: varchar("vendor_no", { length: 20 }).notNull(),
},
(table) => {
	return {
		vendorFk: index("vendor__fk").on(table.vendorNo),
		userHasVendorUserIdVendorNo: primaryKey({ columns: [table.userId, table.vendorNo], name: "user_has_vendor_user_id_vendor_no"}),
		userHasVendorUserIdVendorNoUindex: unique("user_has_vendor_user_id_vendor_no_uindex").on(table.userId, table.vendorNo),
	}
});
...
export const variantWeight = mysqlTable("variant_weight", {
	itemNo: varchar("item_no", { length: 20 }),
	sizeCode: varchar("size_code", { length: 10 }),
...
(table) => {
	return {
		uniqueKey: unique("unique_key").on(table.weight, table.itemNo, table.sizeCode),
	}
});
