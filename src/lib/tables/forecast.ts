...
export const getForecastTableDataMapper = async ({
  ...
  const forecastData = unwrap(
    await getForecastTableData({
      page,
      sorting,
      country,
      brand,
  ...
  const [nextUrl, previousUrl] = buildTableUrl<
    ForecastTableColumns,
    ForecastTableRequest
  >(forecastDataCount, "/api/sales/reports/forecasts/table", {
    page,
    sorting,
    filters,
    country,
  ...
  return {
    data: forecastData.map((data) => {
      ...
      return {
        img_src: [last, Some(itemNo), Some(colorCode)] as [
          Option<string>,
          Option<string>,
          Option<string>,
        ],
