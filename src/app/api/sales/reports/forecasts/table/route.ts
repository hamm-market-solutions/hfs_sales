...
export const GET = async (
  ...
  try {
    ...
    sorting = pipe(
      O.fromNullable(
        searchParams.get("sorting") != "" ? searchParams.get("sorting") : null,
      ),
      O.map((sort) => {
    ...
    filters = pipe(
      O.fromNullable(
        searchParams.get("filters") != "" ? searchParams.get("filters") : null,
      ),
      O.map((filter) => {
    ...
    return resultToResponse(
      Err({ status: 400, message: (error as Error).message, cause: None }),
    );
  }
